...
export class FollowingCollectorService {
  ...
  private async createBenchmark(following: string[]): Promise<void> {
    try {
      ...
      const createData: BenchmarkCreate = { ig_username: this.instagramUsername };
      ...
      for (const followingUsername of following) {
        try {
          const targetData = { ig_username: followingUsername };
          await InstagramService.createTargetApiV1InstagramBenchmarksBenchmarkIdTargetsPost(benchmark.id, targetData);
          addedTargets++;
